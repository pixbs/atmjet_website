...
| `bun run import:legacy <step>`                                       | `airports`, `aircraft`, `vehicles`, `redirects`, `yachts`, `empty-legs` in that order from `legacy`; `reconcile` checks them; `urls --base-url` asks a deployment every legacy URL; `assets` brings the pages' pictures from `legacy/v1` |
