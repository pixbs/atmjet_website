...
5. Make the `conventions` and `ci` checks pass; request the `run-e2e` label for browser tests on the preview deployment. Merge with a merge commit, bottom-up for a stack.
...
- `docs/adr/`: decisions on branching, database migration, i18n, testing, the AI policy, styling and motion, rendering, and the lean conventions (ADR-0008: slices, merge commits, migrations, coverage, enforcement).
