...
- **Payload:** collections in `src/collections`, globals in `src/globals`, blocks in `src/blocks`, shared fields in `src/fields`. After changing the config run `bun run generate:types` and `bun run generate:importmap`; create one migration per finished slice with `bun run migrate:create <name>`, never one per commit, and commit the results. Migration snapshots, `src/payload-types.ts` and the import map are generated files (`.gitattributes`), never edited by hand. Access control is explicit on every collection and covered by `tests/int/access.int.spec.ts`. Payload documentation: https://payloadcms.com/docs.
