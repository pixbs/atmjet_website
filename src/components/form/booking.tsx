...
const Flag = ({ iso, className }: { iso: keyof typeof flags; className?: string }) => {
  ...
  // @ts-ignore
  return C ? <C className={className ?? 'h-4 w-4 rounded-sm'} /> : <span className={`inline-block ${className ?? 'h-4 w-4 rounded-sm'} bg-neutral-900`} />
}
...
export const BookingForm = ({ host, close }: { host: string; close: () => void }) => {
  ...
  const formatDirectionsMd = (l: typeof directions) =>
    l.length
      ? l
          .map((d, i) => {
            const r: string[] = []
            if (d.from) r.push(`*From:* ${mdEscape(d.from)}`)
            if (d.to) r.push(`*To:* ${mdEscape(d.to)}`)
            if (d.date) r.push(`*Date:* ${mdEscape(d.date)}`)
            if (d.returnDate) r.push(`*Return:* ${mdEscape(d.returnDate)}`)
            if (d.passengers) r.push(`*Passengers:* ${d.passengers}`)
            if (d.guests) r.push(`*Guests:* ${d.guests}`)
            if (d.hours) r.push(`*Hours:* ${d.hours}`)
  ...
  const selectCountry = (c: Country) => {
    ...
    setOpen(false)
    setSearchTerm('')
    if (!phoneFieldValue.startsWith(c.code)) setValue('phone_number', `${c.code} `, { shouldDirty: true })
  }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-6' noValidate>
          <div className='flex flex-col gap-1'>
            ...
            <input
              ...
              className={`w-full border-b bg-transparent px-4 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none ${
                errors.name ? 'border-red-500 focus:border-red-500' : 'focus:border-gold border-gray-600'
              }`}
            />
            {touchedFields.name && errors.name && <p className='mt-1 text-xs text-red-500'>{errors.name.message}</p>}
          </div>
            ...
            <div className='relative' ref={dropdownRef}>
              <input
                ...
                className={`w-full border-b bg-transparent py-2 pl-20 pr-4 text-sm text-gray-900 placeholder-gray-400 focus:outline-none ${
                  errors.phone_number ? 'border-red-500 focus:border-red-500' : 'focus:border-gold border-gray-600'
                }`}
              />
              ...
              <button
                ...
                onClick={() => {
                  setOpen((o) => !o)
                  setTimeout(() => dropdownRef.current?.querySelector<HTMLInputElement>('input[data-search]')?.focus(), 0)
                }}
              ...
              {open && (
                <div className='absolute left-0 top-full z-10 mt-[2px] w-full rounded-xl border border-gray-800 bg-gray-900 shadow-[0_6px_20px_rgba(0,0,0,0.08)] overflow-hidden'>
                  ...
                  <ul className='max-h-60 overflow-y-auto'>
                    {filteredCountries.map((c, i) => (
                      <li
                        ...
                        className={`flex cursor-pointer items-center gap-4 px-4 py-3 text-gray-300 last:border-b-0 first:rounded-t-xl last:rounded-b-xl ${
                          i === activeIdx ? 'bg-gray-700' : 'hover:bg-gray-800'
                        }`}
                      >
                        <Flag iso={c.iso} className='h-4 w-4 rounded-sm' />
                        <span className='flex-1 text-sm'>{highlight(regionNames.of(c.iso) || c.name, searchTerm)}</span>
          ...
          <div className='flex flex-col gap-1'>
            ...
            <input
              ...
              type='email'
              className={`w-full border-b bg-transparent px-4 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none ${
                errors.email ? 'border-red-500 focus:border-red-500' : 'focus:border-gold border-gray-600'
              }`}
            />
