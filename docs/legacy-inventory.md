...
The `key-features` cards are worse off than missing. In `uk` their values sit one key behind the English and Russian ones: `card1` holds what `card2` says, `card2` what `card3` says, `card3` what `card4` says, and `card4` what `card1` says — so a Ukrainian page would have drawn all four cards under the wrong headings. The content migration pairs those seven values by what the card says rather than by the key it is filed under (#166).
